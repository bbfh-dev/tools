...
if [[ $1 = "project" ]]; then
    selected=$({ find $GIT -mindepth 2 -maxdepth 2 -type d,l ; find ~/Documents/Godot -mindepth 1 -maxdepth 1 -type d,l ; find ~/Public/bookmarks -mindepth 1 -maxdepth 1 -type d,l ; } | fzf --scheme=path --algo=v2 --layout=reverse-list --margin 10% --padding 10%)
    if [[ -z $selected ]]; then
        return
    fi
